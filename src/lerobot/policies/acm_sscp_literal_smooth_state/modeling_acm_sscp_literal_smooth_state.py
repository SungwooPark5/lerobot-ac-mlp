"""S3 — ACM (Mamba-1) literal carry + SSM state-continuity smoothness.
Port of acm2_sscp_literal_smooth_state.

Thin subclass of ACMSSCPLiteralSmoothPolicy. Same model and inference; only the chunk-pair
...
class ACMSSCPLiteralSmoothStatePolicy(ACMSSCPLiteralSmoothPolicy):
    """Literal carry + GT-matched seam loss + carry-vs-fresh state-continuity term."""

    config_class = ACMSSCPLiteralSmoothStateConfig
    name = "acm_sscp_literal_smooth_state"
    ...
    def _forward_chunk_pair(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if lam <= 0.0 and sw <= 0.0:
            ...
            return ACMSSCPLiteralPolicy._forward_chunk_pair(self, batch)
