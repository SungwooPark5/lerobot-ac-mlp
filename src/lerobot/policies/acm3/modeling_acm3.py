...
"""Action Chunking Mamba-3 (ACM-3) Policy

Replaces the Mamba-2 SSD decoder in ACM-2 with the Mamba-3 inference-first SSM block.
Mamba-3 drops the 1D convolutional component, integrates RoPE inside the SSM, and exposes
an optional MIMO (Multiple-Input Multiple-Output) mode for stronger expressivity per
parameter. Recurrent O(1) per-step inference is enabled via layer_idx state caching.
"""
...
class ACM3Policy(PreTrainedPolicy):
    config_class = ACM3Config
    name = "acm3"
...
class ACM3(nn.Module):
    """Action Chunking Mamba-3: Uses Mamba-3 layers as the decoder.

    Architecture is identical to ACM-2 except the decoder uses Mamba3 (inference-first SSM
    with optional MIMO mode and integrated RoPE) instead of Mamba2 (SSD). The encoder
    (Transformer) and VAE encoder are unchanged.
    """

    def __init__(self, config: ACM3Config):
        super().__init__()
        self.config = config
    ...
    def _reset_parameters(self):
        """Xavier-uniform init for transformer params; preserve Mamba3's internal init."""
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Forward pass through transformer encoder and Mamba-3 decoder.
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: ACM3Config, is_vae_encoder: bool = False):
        super().__init__()
...
class Mamba3ACMDecoder(nn.Module):
    """Mamba-3 inference-first decoder for ACM-3.

    Key differences from Mamba-2:
    - No 1D convolutional component (d_conv removed)
    - RoPE integrated inside the SSM (rope_fraction)
    - Optional MIMO mode with configurable rank (is_mimo, mimo_rank)
    - Recurrent O(1) per-step inference path via layer_idx
    """
    ...
    def forward(
        ...
        # Concatenate encoder context + decoder queries.
        combined_seq = torch.cat([encoder_out, x], dim=1)

        # Apply Mamba-3 SSD layers.
        for layer in self.layers:
            combined_seq = layer(combined_seq)

        # Extract the predicted action tokens (last chunk_size tokens).
