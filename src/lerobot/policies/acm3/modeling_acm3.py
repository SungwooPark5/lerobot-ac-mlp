...
class Mamba3ACMDecoder(nn.Module):
    """Mamba-3 decoder for ACM-3 with optional BiMamba and action self-attention."""
    ...
    def __init__(self, config: ACM3Config):
        ...
        def make_mamba3_layer(layer_idx: int):
            return Mamba3(
        ...
        if self.use_bimamba_decoder:
            self.forward_layers = nn.ModuleList(
                [make_mamba3_layer(i) for i in range(config.n_decoder_layers)]
            )
            self.backward_layers = nn.ModuleList(
                [make_mamba3_layer(i + config.n_decoder_layers) for i in range(config.n_decoder_layers)]
            )
            self.layers = None
        else:
