...
"""Action Chunking Mamba-2 (ACM-2) Policy

Replaces the Mamba-1 selective scan decoder in ACM with Mamba-2's SSD (State Space Duality) layer.
Mamba-2 uses a structured state space decomposition that enables more efficient computation
while maintaining the selective scan capability.
"""
...
class ACM2(nn.Module):
    """Action Chunking Mamba-2: Uses Mamba-2 SSD layers as the decoder.

    Architecture is identical to ACM except the decoder uses Mamba2 instead of Mamba.
    The encoder (Transformer) and VAE encoder remain unchanged.
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Forward pass through transformer encoder and Mamba-2 decoder.
...
class Mamba2ACMDecoder(nn.Module):
    """Mamba-2 SSD decoder for ACM-2.

    Uses Mamba2 (State Space Duality) layers instead of Mamba-1's selective scan.
    Key differences from Mamba-1:
    - Uses structured state space decomposition (SSD) for O(N) computation
    - Multi-head structure with configurable head dimension
    - Typically uses larger d_state (64 vs 16) for better expressiveness
    """
    ...
    def __init__(self, config: ACM2Config):
        ...
        if not HAS_MAMBA2:
            raise ImportError(
                "mamba-ssm >= 2.0 is required for Mamba2. "
