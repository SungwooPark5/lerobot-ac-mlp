...
from lerobot.policies.acm_accel_loss._legacy_configuration_acm_accel import ACMAccelConfig
from lerobot.policies.pretrained import PreTrainedPolicy
from lerobot.utils.constants import ACTION, OBS_ENV_STATE, OBS_IMAGES, OBS_STATE

class ACMAccelPolicy(PreTrainedPolicy):
    config_class = ACMAccelConfig
    name = "acm_accel_loss"

    def __init__(
        self,
        config: ACMAccelConfig,
    ):
        """
        Args:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # ------------------------------------------------------------
        # Final action acceleration loss
        #
        # actions_hat is final output action.
        #
        # accel[t] = a[t+2] - 2a[t+1] + a[t]
        #
        # action_accel_front_steps = 0:
        #   use full chunk
        #
        # action_accel_front_steps > 0:
        #   use only first N steps
        # ------------------------------------------------------------
        action_accel_loss = None
        ...
        if action_accel_weight > 0.0 and actions_hat.shape[1] >= 3:
        ...
        if action_accel_loss is not None:
            loss = loss + action_accel_weight * action_accel_loss
...
class ACM(nn.Module):
    ...
    def __init__(self, config: ACMAccelConfig):
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: ACMAccelConfig, is_vae_encoder: bool = False):
        super().__init__()
