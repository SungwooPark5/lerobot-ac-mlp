"""Configuration for eunji ACM2 + literal SSD state carry + boundary-smoothness loss.

Adds an explicit chunk-boundary continuity (C1/C2) loss on top of the literal-carry
chunk-continuation (CC) training, ported from ecd-v15's acm3_sscp_smooth. The literal
state carry already gives chunk n+1 the *information* about chunk n; this loss gives the
*incentive* to use it for a smooth seam.
...
sscp_smooth_weight = 0.0  ->  identical behaviour to acm2_sscp_literal (invariant / safety).
...
This config subclasses ACM2SSCPLiteralConfig, so it inherits use_bimamba_decoder: set it
...
class ACM2SSCPLiteralSmoothConfig(ACM2SSCPLiteralConfig):
    """ACM2 literal carry + explicit GT-matched chunk-boundary continuity loss.

    Architecture / carry are identical to acm2_sscp_literal (same stateful decoder, same
    literal (conv, ssm) handoff). Only the training objective gains the seam term.
    """

    # lambda for the boundary continuity loss. 0 = off (== acm2_sscp_literal). Sweep this.
