"""eunji ACM2 + literal SSD state carry + explicit chunk-boundary continuity loss.

Ported from ecd-v15's acm3_sscp_smooth, adapted to the literal-state carry base. The base
literal carry gives chunk n+1 the *information* about chunk n; this policy adds a GT-matched
boundary-continuity loss to the chunk-continuation (CC) training to give the *incentive* for
a smooth seam.
...
ACM2SSCPLiteralPolicy unchanged. sscp_smooth_weight = 0 reproduces acm2_sscp_literal exactly.
...
class ACM2SSCPLiteralSmoothPolicy(ACM2SSCPLiteralPolicy):
    """ACM2 + literal carry with a chunk-boundary continuity loss added to CC training.

    Same model and inference as ACM2SSCPLiteralPolicy; only `_forward_chunk_pair` is
    ...
    def __init__(self, config: ACM2SSCPLiteralSmoothConfig):
    ...
    def _forward_chunk_pair(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # lambda<=0 (weight 0 OR warmup not started) → identical to acm2_sscp_literal.
        if lam <= 0.0:
