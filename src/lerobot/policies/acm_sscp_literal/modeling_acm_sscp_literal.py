"""ACM (Mamba-1) + literal selective-scan state carryover (SSCP), with optional
boundary-time carry fusion. Port of acm2_sscp_literal to the Mamba-1 backbone.

Backbone = this branch's ACM (Transformer encoder -> Mamba-1 decoder, with the optional
native BiMamba / action self-attention). This policy carries Mamba-1's actual recurrent
state across chunk boundaries, plus the depthwise-conv state, so the scan continues
exactly from where the previous chunk ended.

Per-layer state = (conv_state, ssm_state):
  conv_state : (B, d_inner, W-1)       last (W-1) conv inputs (causal tail)
  ssm_state  : (B, d_inner, d_state)   selective-scan recurrent state

Initial-state injection (Mamba-1 specific): unlike Mamba-2's SSD kernel, Mamba-1's
`selective_scan_fn` has no `initial_states` argument. But Mamba-1's A is diagonal, so the
recurrence h_t = exp(delta_t A) h_{t-1} + delta_t B_t x_t gives the initial state a
closed-form (exact, differentiable) contribution on top of the zero-init scan:

  h_t     = h_t^{zero}     + exp(A * cumsum(delta)_t) * h_0
  y_t     = y_t^{zero}     + C_t . (exp(A * cumsum(delta)_t) * h_0)

`mamba1_stateful_forward` runs the standard kernel with zero init and adds this term, so
the carried scan is mathematically identical to never having reset the SSM.
...
the cross-chunk state; the *backward* (flipped) scan is a stateless, intra-chunk refinement
of the already-planned action block (no cross-chunk carry, since a reverse-time state across
...
is treated at each boundary — see configuration_acm_sscp_literal for the "none"/"ema"/"mlp"/
...
# Mamba-1 selective-scan kernel (no initial_states support; see closed-form injection above).
try:
...
from lerobot.policies.acm.modeling_acm import ACM, ACTTemporalEnsembler
...
# ── Stateful Mamba-1 forward (exposes conv/ssm state) ─────────────────────────────

def _initial_state_correction(A: Tensor, delta: Tensor, C: Tensor, h0: Tensor):
    """Exact contribution of an initial SSM state to the diagonal selective scan.

    A: (d_inner, N) fp32, negative. delta: (B, d_inner, L) fp32, post-softplus.
    C: (B, N, L) fp32. h0: (B, d_inner, N) fp32.
    Returns (y_extra (B, d_inner, L), h_extra_last (B, d_inner, N)), both fp32:
      decay_t = exp(A * cumsum(delta)_t);  y_extra_t = C_t . (decay_t * h0).
    Valid because the diagonal per-step transitions commute:
    prod_s exp(delta_s A) = exp(A sum_s delta_s).
    """
    decay = torch.exp(A.unsqueeze(0).unsqueeze(-1) * delta.cumsum(dim=-1).unsqueeze(2))  # (B, d, N, L)
    y_extra = torch.einsum("bdnl,bdn,bnl->bdl", decay, h0, C)
    return y_extra, decay[..., -1] * h0


def mamba1_stateful_forward(m, u: Tensor, initial_state=None, return_state: bool = False):
    """Replicate Mamba(-1).forward (non-fused path) exposing initial/return states.
    ...
    The z-gating is applied manually (outside the kernel) so the closed-form
    initial-state correction can be added to the raw scan output first.
    """
    if not HAS_SELECTIVE_SCAN:
        raise ImportError(
            "selective_scan_fn unavailable. Install mamba-ssm (pip install mamba-ssm)."
        )
    conv_state_in, ssm_state_in = (initial_state if initial_state is not None else (None, None))
    _, seqlen, _ = u.shape

    xz = m.in_proj(u).transpose(1, 2)  # (B, 2*d_inner, L)
    x, z = xz.chunk(2, dim=1)  # (B, d_inner, L) each

    # Depthwise causal conv with carried conv state.
    x, conv_state_final = _causal_conv_step(m.conv1d, x, conv_state_in, return_state)
    x = m.act(x)  # SiLU, (B, d_inner, L)

    x_dbl = m.x_proj(rearrange(x, "b d l -> (b l) d"))  # (B*L, dt_rank + 2*N)
    dt, Bmat, Cmat = torch.split(x_dbl, [m.dt_rank, m.d_state, m.d_state], dim=-1)
    dt = rearrange(F.linear(dt, m.dt_proj.weight), "(b l) d -> b d l", l=seqlen).contiguous()
    Bmat = rearrange(Bmat, "(b l) n -> b n l", l=seqlen).contiguous()
    Cmat = rearrange(Cmat, "(b l) n -> b n l", l=seqlen).contiguous()
    A = -torch.exp(m.A_log.float())  # (d_inner, N)

    need_last = return_state or ssm_state_in is not None
    ret = selective_scan_fn(
        x,
        dt,
        A,
        Bmat,
        Cmat,
        m.D.float(),
        z=None,  # gate manually below (after the initial-state correction)
        delta_bias=m.dt_proj.bias.float(),
        delta_softplus=True,
        return_last_state=need_last,
    )
    if need_last:
    ...
    if ssm_state_in is not None:
        # dt is pre-bias / pre-softplus; the kernel applies both, so mirror that here.
        delta = F.softplus(dt.float() + m.dt_proj.bias.float().view(1, -1, 1))  # (B, d, L)
        y_extra, h_extra = _initial_state_correction(A, delta, Cmat.float(), ssm_state_in.float())
        y = y + y_extra.to(y.dtype)
        ssm_state_final = ssm_state_final + h_extra.to(ssm_state_final.dtype)

    y = y * m.act(z)  # z-gating (what the kernel does internally when z is passed)
    out = m.out_proj(rearrange(y, "b d l -> b l d"))
    if return_state:
...
class CarryStateFusion(nn.Module):
    ...
    The Mamba-1 ssm_state has shape (B, D, N) — D d_inner channels, N d_state. Both variants
    act at the d_state granularity, SHARED across the D channels, so the gate is
    per-state-channel and the parameter count matches the Mamba-2 version (does not scale
    with d_inner).

    mlp   — h' = h + MLP(h), residual with zero-init output layer (starts as literal carry).
    gated — observation-driven boundary correction. G = sigma(g([mean_D(h); e_obs]) + b0),
            broadcast over D. `carry_gate_mode` selects the correction:
              reset    h' = (1-G)*h
              replace  h' = (1-G)*h + G*S(e_obs)
              residual h' = h + G*D(e_obs)
    """

    def __init__(self, config: ACMSSCPLiteralConfig):
        super().__init__()
        self.mode = config.carry_fusion
        self.gate_mode = getattr(config, "carry_gate_mode", "replace")  # reset|replace|residual
        self.d_inner = config.dim_model * config.mamba_expand
        ...
        if self.mode == "mlp":
            ...
            def make_gate():
                out = nn.Linear(hidden, N)  # per-state-channel gate, broadcast over D
            ...
            if self.gate_mode in ("replace", "residual"):
                def make_target():
                    s = nn.Linear(D, N)  # obs -> per-channel target/delta, broadcast over D
    ...
    def forward(self, states, e_obs: Tensor):
        ...
        for i, (conv, ssm) in enumerate(states):
            b, d, n = ssm.shape
            assert d == self.d_inner and n == self.d_state, (
                f"ssm_state shape {tuple(ssm.shape)} != configured (D={self.d_inner}, N={self.d_state})"
            )
            ssm_in = ssm.to(e_obs.dtype)
            if self.mode == "mlp":
                ssm_out = ssm_in + self.proj[i](ssm_in)
            else:  # gated — per-state-channel gate, broadcast over the d_inner channels (D)
                pooled = ssm_in.mean(dim=1)  # (B, N)
                gate = torch.sigmoid(self.gate[i](torch.cat([pooled, e_obs], dim=-1)))
                gate = gate.view(b, 1, n)  # broadcast over D
                if self.gate_mode == "reset":
                    ssm_out = (1.0 - gate) * ssm_in  # gate toward 0 (= ACT)
                elif self.gate_mode == "residual":
                    delta = self.target[i](e_obs).view(b, 1, n)
                    ssm_out = ssm_in + gate * delta  # additive obs correction
                else:  # replace
                    target = self.target[i](e_obs).view(b, 1, n)
...
# ── Decoder with literal per-layer state handoff (mirrors acm2: plain / BiMamba) ──

class Mamba1LiteralSSCPDecoder(nn.Module):
    """The ACM Mamba-1 decoder made stateful: each layer's recurrent (conv, ssm) state is
    carried across chunks via the forward scan. Mirrors the ACM decoder options.
    ...
    Optional action self-attention is applied to the last-K tokens, exactly as in ACM.
    ...
    def forward(
        ...
        if self.use_bimamba_decoder:
            ...
            for i, layer in enumerate(self.forward_layers):
                ...
                forward_seq, st = mamba1_stateful_forward(
            ...
            for layer in self.backward_layers:
                backward_seq = mamba1_stateful_forward(
                    layer, backward_seq, initial_state=None, return_state=False
                )
            ...
            for i, layer in enumerate(self.layers):
                init = carry[i] if carry is not None else None
                combined_seq, st = mamba1_stateful_forward(
...
# ── Model (subclasses ACM; swaps decoder for the stateful one) ─────────────────────

class ACMSSCPLiteral(ACM):
    """ACM with the decoder replaced by a literal-state-handoff decoder.
    ...
    def __init__(self, config: ACMSSCPLiteralConfig):
        ...
        self.decoder = Mamba1LiteralSSCPDecoder(config)
        self.carry_fusion = (
        ...
        # time). Mamba params are protected by id in ACM._reset_parameters; carry_fusion is
        # not under encoder/decoder so its custom zero-init is preserved.
        self._reset_parameters()

    def _encode(self, batch: dict[str, Tensor], batch_size: int):
        """ACM.forward encode block: returns encoder_out (S,B,D), pos_embed (S,B,D),
        mu, log_sigma_x2."""
...
class ACMSSCPLiteralPolicy(PreTrainedPolicy):
    """ACM (Mamba-1) + literal recurrent state carryover (with optional carry fusion)."""

    config_class = ACMSSCPLiteralConfig
    name = "acm_sscp_literal"
