"""ACM (Mamba-1) + literal state carry + explicit chunk-boundary continuity loss.
Port of acm2_sscp_literal_smooth.

The base literal carry gives chunk n+1 the *information* about chunk n; this policy adds a
GT-matched boundary-continuity loss to the chunk-continuation (CC) training to give the
*incentive* for a smooth seam.

Why GT-matched (not jitter->0): the two CC chunks are consecutive real chunks, so the GT
transition a_n[-1] -> a_{n+1}[0] is already smooth. We match the *predicted* seam finite-
differences (velocity + acceleration) to the *GT* ones, instead of pushing the predicted jump
to zero — so the objective is aligned with the task (won't make the motion sluggish / hurt SR)
while still flattening the boundary spike.

Everything else — model, literal carry, inference (select_action) — is inherited from
ACMSSCPLiteralPolicy unchanged. sscp_smooth_weight = 0 reproduces acm_sscp_literal exactly.
Set use_bimamba_decoder=True (inherited) to combine BiMamba + carry + smoothness.
"""
...
class ACMSSCPLiteralSmoothPolicy(ACMSSCPLiteralPolicy):
    """ACM + literal carry with a chunk-boundary continuity loss added to CC training.

    Same model and inference as ACMSSCPLiteralPolicy; only `_forward_chunk_pair` is
    overridden to add the seam-smoothness term. Supports lambda warmup, C1/C2 order, and an
    inference-mode (free-latent) variant of the seam loss.
    """

    config_class = ACMSSCPLiteralSmoothConfig
    name = "acm_sscp_literal_smooth"

    def __init__(self, config: ACMSSCPLiteralSmoothConfig):
        super().__init__(config)
        # training-step counter for lambda warmup (persists across resume).
    ...
    def _smooth_lambda(self) -> float:
        """Current lambda: 0 before warmup_start, ramps to sscp_smooth_weight over warmup_steps."""
    ...
    def _free_latent_seam_actions(self, batch_n, batch_n1):
        ...
        Setting the model to eval makes _encode take the latent=0 branch (no VAE teacher
        forcing) and skips the training assert — matching the actual inference boundary,
        including the real literal (conv, ssm) state handoff. Gradients still flow (only
        dropout/BN behaviour changes).
        """
        was = self.model.training
        self.model.eval()
        try:
            a_n, _, states_n = self.model(batch_n, carry=None, return_state=True)
            carry_f = detach_states(states_n)
            a_n1, _, _ = self.model(batch_n1, carry=carry_f, return_state=True)
        finally:
    ...
    def _forward_chunk_pair(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        # lambda<=0 (weight 0 OR warmup not started) → identical to acm_sscp_literal.
        if lam <= 0.0:
        ...
        loss_n, loss_dict_n, states_n, act_n = self._forward_single(batch_n, carry=None)
        ...
        loss_n1, loss_dict_n1, _, act_n1 = self._forward_single(batch_n1, carry=carry)
        ...
        # ── Combined loss (lambda from warmup schedule) ───────────────────────────
