"""ACM2 + Intra-Chunk Phase Embedding (ICPE) — Mamba-2 (SSD) port of acm3_icpe.
...
Key change vs ACM2:
  ...
  For Mamba-2, this matters because position i's hidden state h_i is the SSD
  accumulation of h_0 .. h_{i-1}. Telling the model "you are at position i/K"
  calibrates uncertainty that otherwise grows toward the chunk END.
...
Also introduces Mamba2ICPEDecoder that accepts an optional `carry` tensor
...
# ── Mamba-2 decoder with ICPE-aware carry support ──────────────────────────────

class Mamba2ICPEDecoder(nn.Module):
    """Mamba-2 SSD decoder that optionally accepts a carry token from the previous chunk.

    Interface identical to Mamba2ACMDecoder, plus:
        carry (B, 1, D): if provided, prepended before the queries in the combined
    ...
    by the parent ACM2ICPE.forward() before calling this decoder.
    """

    def __init__(self, config: ACM2ICPEConfig):
        super().__init__()
        if not HAS_MAMBA2:
            raise ImportError(
...
class ACM2ICPE(nn.Module):
    """ACM2 with Intra-Chunk Phase Embedding and optional carry token support."""
    ...
    def __init__(self, config: ACM2ICPEConfig):
        ...
        # ── Mamba-2 ICPE decoder ──────────────────────────────────────────────
        self.decoder = Mamba2ICPEDecoder(config)
    ...
    def forward(
        ...
        # Combine: pos_embed + ICPE  → (K, 1, D) broadcast to (K, B, D) inside decoder
...
class ACM2ICPEPolicy(PreTrainedPolicy):
    config_class = ACM2ICPEConfig
    name = "acm2_icpe"
