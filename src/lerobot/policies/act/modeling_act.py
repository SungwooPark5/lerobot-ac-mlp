...
class ACTPolicy(PreTrainedPolicy):                                                                                                                      
  ...
  def __init__(                                                                                                                                       
      ...
      elif config.use_learned_ensemble:   
  ...
  def reset(self):                                                                                                                                    
      ...
      elif self.config.use_learned_ensemble:
          self.learned_ensembler.reset()                                                                                                              
      else:                                                                                                                                           
  ...
  def select_action(self, batch: dict[str, Tensor]) -> Tensor:
      ...
      if self.config.use_learned_ensemble:                                                                                                            
          # Need both actions and logits, so call the model directly rather than through
          # predict_action_chunk (which discards logits).                                                                                             
  ...
  def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:                                                                                 
      ...
      if self.config.use_learned_ensemble:
          return self._forward_learned_ensemble(batch)                                                                                                
                                                                                                                                                      
      actions_hat, _, (mu_hat, log_sigma_x2_hat) = self.model(batch)
  ...
  def _forward_learned_ensemble(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
      L = self.config.ensemble_size                                                                                                                       
      chunk_size = self.config.chunk_size
      total_T = chunk_size + L - 1                                                                                                                        
                                                                                                                                                          
      all_actions, all_weight_logits = [], []
      mus, log_sigmas = [], []                                                                                                                            
                                                                                                                                                          
      for k in range(L):
          sub_batch: dict[str, Tensor] = {}                                                                                                               
          if OBS_STATE in batch:                                                                                                                          
              sub_batch[OBS_STATE] = batch[OBS_STATE][:, k]
          if OBS_ENV_STATE in batch:                                                                                                                      
              sub_batch[OBS_ENV_STATE] = batch[OBS_ENV_STATE][:, k]
          if OBS_IMAGES in batch:                                                                                                                         
              sub_batch[OBS_IMAGES] = [img[:, k] for img in batch[OBS_IMAGES]]
          if ACTION in batch:                                                                                                                             
              sub_batch[ACTION] = batch[ACTION][:, k : k + chunk_size]
          if "action_is_pad" in batch:                                                                                                                    
              sub_batch["action_is_pad"] = batch["action_is_pad"][:, k : k + chunk_size]                                                                  
                                                                                                                                                          
          actions_k, logits_k, (mu_k, log_sigma_k) = self.model(sub_batch)                                                                                
          all_actions.append(actions_k)            # (B, chunk_size, action_dim)
      ...
      mask = (~batch["action_is_pad"]) & coverage.unsqueeze(0)  # (B, total_T)                                                                            
...
class ACTTemporalEnsembler:                                                                                                                             
  def __init__(self, temporal_ensemble_coeff: float, chunk_size: int) -> None:
      ...
      # ↓ 진단용 chunk logging (단순화: 매 update마다 개별 파일)                                  
      import os                                                                                   
      self._log_dir = os.environ.get("LOG_CHUNKS_DIR")                                            
      self._log_step = 0                                                                          
      if self._log_dir:                                                                           
          os.makedirs(self._log_dir, exist_ok=True)
          print(f"[mode_collapse_diag] logging chunks to {self._log_dir}")                        
  ...
  def update(self, actions: Tensor) -> Tensor:                                                                                                        
      ...
      if self._log_dir:                                                                       
          import numpy as np                                                                  
          np.save(                                                                            
              f"{self._log_dir}/chunk_{self._log_step:06d}.npy",                              
...
class ACTLearnedTemporalEnsembler:                                                                                                                      
    def __init__(self, chunk_size: int, ensemble_size: int) -> None:
        self.chunk_size = chunk_size                                                                                                                    
        self.L = ensemble_size
        self.reset()                                                                                                                                    
...
class ACT(nn.Module):                                                                                                                                   
  ...
  def __init__(self, config: ACTConfig):                                                                                                              
      ...
      # Separate decoder dedicated to weight (logit) prediction so it doesn't          
      # compete with action head for shared decoder features.                          
      if self.config.use_learned_ensemble:                                             
          self.weight_decoder = ACTDecoder(config)                                     
          self.weight_decoder_pos_embed = nn.Embedding(config.ensemble_size, config.dim_model)   
      ...
      # Per-position weight (logit) head used for learned temporal ensembling.                                                                        
      # One scalar per chunk position. Softmax is applied later when overlapping predictions                                                          
      # are combined (at inference in the ensembler, at training in ACTPolicy.forward).                                                               
      if self.config.use_learned_ensemble:                                                                                                            
          self.weight_head = nn.Linear(config.dim_model, 1)                                                                                           
                                                                                                                                                      
      self._reset_parameters()                                                                                                                        
                                                                                                                                                      
  def _reset_parameters(self):                                                             
      """Xavier-uniform initialization of the transformer parameters as in the original 
  code."""                                                                                 
      iters = [self.encoder.parameters(), self.decoder.parameters()]
      if self.config.use_learned_ensemble:
          iters.append(self.weight_decoder.parameters())
      for p in chain(*iters):             
          if p.dim() > 1:                       
              nn.init.xavier_uniform_(p) 
                                                                                                                                                      
  def forward(                                                                                                                                        
      self, batch: dict[str, Tensor]                                                                                                                  
  ) -> tuple[Tensor, Tensor | None, tuple[Tensor, Tensor] | tuple[None, None]]:                                                                       
      ...
      Returns:
          ...
          logits: (B, chunk_size) per-position scalars for learned temporal ensembling,
              or None when `use_learned_ensemble` is disabled.                                                                                        
          (mu, log_sigma_x2): latent PDF parameters, both (B, latent_dim), or (None, None)
              when the VAE branch is not used in this pass.                                                                                           
      """                                                                                                                                             
      ...
      if self.config.use_learned_ensemble:                                                                                                                    
          L = self.config.ensemble_size
          weight_decoder_in = torch.zeros(
              (L, batch_size, self.config.dim_model),                                                                                                         
              dtype=encoder_in_pos_embed.dtype,
              device=encoder_in_pos_embed.device,                                                                                                             
          )                                                                                                                                                   
          weight_decoder_out = self.weight_decoder(                                                                                                           
              weight_decoder_in,                                                                                                                              
              encoder_out,
              encoder_pos_embed=encoder_in_pos_embed,
              decoder_pos_embed=self.weight_decoder_pos_embed.weight.unsqueeze(1),                                                                            
          )                                                                                                                                                   
          weight_decoder_out = weight_decoder_out.transpose(0, 1)  # (B, L, D)                                                                                
