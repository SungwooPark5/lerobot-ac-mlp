"""ACM2 + SSM State Carryover Protocol (SSCP) — Mamba-2 (SSD) port of acm3_sscp.

Terminology (important for the paper): SSCP does NOT copy Mamba-2's literal SSD
recurrent state across the chunk boundary. Instead it carries a single *summary
token* (the previous chunk's terminal decoder output) and lets the SSM *re-derive*
a non-zero hidden state by scanning that token before the queries. Describe it as
"summary-token state warm-up", not "literal hidden-state transfer". The literal
recurrent-state carry (and the v8 carry-fusion family) lives in acm2_sscp_literal.

How SSCP works (inference):
  1. After chunk n, extract the Mamba-2 decoder's LAST OUTPUT TOKEN:
       carry_n = decoder_out[:, -1:, :]   # (B, 1, D)
  2. Before chunk n+1's action queries, insert carry_n into the combined sequence
     (default placement "pre_query": [encoder_out, carry, queries]).
  3. The K action tokens are extracted from the LAST K positions of combined_out.

Why this is SSM-specific (not applicable to ACT Transformer):
  - Mamba-2 processes carry_n sequentially, so its hidden state propagates into all
    subsequent positions. In a Transformer carry_n is merely another attention key.
"""
...
from lerobot.policies.acm2.configuration_acm2 import ACM2Config  # noqa: F401  (registration safety)
from lerobot.policies.acm2.modeling_acm2 import (
    ACTEncoder,
...
# ── Mamba-2 decoder with summary-token carry support ──────────────────────────

class Mamba2SSCPDecoder(nn.Module):
    """Mamba-2 SSD decoder that optionally accepts a carry token from the previous chunk.

    carry (B, 1, D): if provided, inserted before the queries so the SSM initialises
                     from the previous chunk's hidden-state context.
    """

    def __init__(self, config: ACM2SSCPConfig):
        super().__init__()
        if not HAS_MAMBA2:
            raise ImportError(
...
class ACM2SSCP(nn.Module):
    """ACM2 with SSCP summary-token carry support."""
    ...
    def __init__(self, config: ACM2SSCPConfig):
        ...
        # ── Mamba-2 SSCP decoder ──────────────────────────────────────────────
        self.decoder = Mamba2SSCPDecoder(config)
...
class ACM2SSCPPolicy(PreTrainedPolicy):
