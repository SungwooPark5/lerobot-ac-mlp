...
세 노트북은 **내용이 같고 노드 문자만 다르다.** 학습으로 배정되든 eval 로 배정되든 하나로 된다
(6·7번 셀이 배정된 역할이 아니면 스스로 건너뛴다).

역할은 `X.suggest()` 가 정한다 — **ready eval 큐를 먼저 포화시키고 남는 노드만 학습에 준다.**
...
(8/24 밤 실측: 4잡을 2 GPU 로 2배치 = 약 16h. 처음 잡았던 15h/잡 추정은 과했다).
...
**ACT 가 전 K(20/50/100/150) 학습돼 있으므로 TE 축 14셀이 전부 재학습 없이 돈다.**
그래서 8/25 시점의 배정은 **세 노드 전부 eval** 이다 (ready eval 61셀 vs 남은 학습 3잡).
