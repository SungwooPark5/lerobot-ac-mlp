...
### 8/28 — 지금 할 일: **stride sweep** (해준님이 8/25 은지님께 약속)
...
**동일 비용 우위**다. 그런데 봉우리 양옆(s5·s15·s25)이 비어 봉우리를 감싸지 못했다
(ACT 가 s5 에서 역전할 가능성이 아직 배제 안 됨) -> exp5 의 P1 이 이걸 채운다.

같은 세팅(K=100 s10)에서 확보된 부수 근거:

| 실험 | 결과 | 의미 |
|---|---|---|
| **스캔 ablation** | plain 56.8 / scan_same(2배 파라미터) 56.0 / BiMamba **65.0** | 성능은 파라미터가 아니라 **역방향 스캔** 덕 (+8.2). 교수님 요청 실험, n=500 |
| backbone | r18 65.0 vs r50 51.0 (−14.0) | ResNet18 유지 근거 |
| decoder layer | L1 59.1 / L2 63.4 / L4 48.6 | 2층 |

그리고 **K sweep @ stride=10 고정**이 메인 그림이 된다. 현재 이 축에 K=50(ACT 67.6 /
BiMamba 57.8)·K=100(42.5 / 65.0) 두 점뿐이라 크로스오버가 50~100 사이라는 것만 알고
곡선이 없다. ACT ckpt 는 전 K 에 있으므로 **재학습 없이 eval 만**으로 채워진다.

carry 는 메인에서 빼고 ablation 으로 남기는 쪽이 데이터에 맞는다: TE 축 전 K 에서 기여가
노이즈(−1.1~+0.9)고 K=100 s10 에선 −4.6 으로 오히려 해친다. 다만 carry-only 는 ACM2 대비
s10 +3.2 / s100 +9.2 로 혼자서는 분명히 효과가 있다 -> *"두 메커니즘은 보완재가 아니라
대체재"* (은지님 8/18 결론)를 ablation 한 절로 쓴다.
